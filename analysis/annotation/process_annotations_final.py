...
def main():
    ...
    crop_top_left_x = 73
    crop_top_left_y = 65
    crop_bottom_right_x = 1832
    crop_bottom_right_y = 960
    generate_video = False
    no_crop = False

    root_address = "exp_data/exp_data_4"
    annotation_folder = "out_annotation_csv"
    ...
    net_folders = ['packet_loss', 'communication_loss', 'delay', 'no_fault']
    ...
    for folder in net_folders:
        ...
        for sub_folder in os.listdir(path): 
            ...
            if sub_folder[0] == 'c': 
                ...
                name = "free"+str(num)
            csv_subpath = os.path.join(sub_path, annotation_folder)
            os.makedirs(csv_subpath, exist_ok=True)
            
            csv_path = os.path.join(sub_path, name + "_annotation.csv")
            if not os.path.exists(csv_path):
                print(f"CSV file not found: {csv_path}. Skipping...")
                continue
            video_path = os.path.join(sub_path, name + ".mp4")

            output_mp_csv = os.path.join(csv_subpath, name + "_annotation_processed.csv")
            output_error_csv = os.path.join(csv_subpath, name + "_annotation_error.csv")
            output_segment_csv = os.path.join(csv_subpath, name + "_annotation_segment.csv")
            output_error_segment_csv = os.path.join(csv_subpath, name + "_annotation_error_segment.csv")
            output_video = os.path.join(csv_subpath, name + "_annotated.mp4")

            processor = AnnotationProcessor(csv_path, video_path)
            processor.parse_csv()
            #processor.fill_gaps()
            ...
            processor.save_csv(mp_annotations, output_mp_csv, mp_fieldnames)
            processor.save_csv(error_annotations, output_error_csv, error_fieldnames)
            processor.save_segment_csv(segment_annotations, output_segment_csv, mp_segment_fieldnames)
            processor.save_segment_csv(error_segments, output_error_segment_csv , error_segment_fieldnames)
            ...
            if generate_video:
                ...
                if no_crop:
                    ...
                    crop_coords = (crop_top_left_x, crop_top_left_y, 
                                crop_bottom_right_x, crop_bottom_right_y)
                    print(f"Using crop coordinates: top-left({crop_top_left_x}, {crop_top_left_y}), "
                        f"bottom-right({crop_bottom_right_x}, {crop_bottom_right_y})")

                processor.create_annotated_video(mp_annotations, output_video, crop_coords)
