...
def plot_the_stacked_bar_chart(data_robot, data_console, transfer_ml_free_robot, transfer_ml_free_console, std_robot, std_console):
    ...
    plt.figure(figsize=(18, 10), constrained_layout=False)
    ...
    for i in range(data_console.shape[1]):
        ...
        for j in range(len(net_conditions)):
            if data_robot[j, i] > 20:
                plt.text(x1[j], bottom1[j] + data_robot[j, i]/2, f"{data_robot[j, i]:.0f}", ha='center', va='center', fontsize=20, weight='bold')
        ...
        for j in range(len(net_conditions)):
            if data_console[j, i] > 20:
                plt.text(x2[j], bottom2[j] + data_console[j, i]/2, f"{data_console[j, i]:.0f}", ha='center', va='center', fontsize=20, weight='bold')
    ...
    # Add double arrows for PLM3, DLM3, CLM3 to show difference between robot and console
    arrow_conditions = [3, 6, 9]  # PLM3, DLM3, CLM3 indices
    arrow_labels = ['PLM3', 'DLM3', 'CLM3']
    
    for i, condition_idx in enumerate(arrow_conditions):
        robot_height = np.sum(data_robot[condition_idx, :])
        console_height = np.sum(data_console[condition_idx, :])
        
        # Position the arrow to the left of the console bar
        arrow_x = x2[condition_idx] - bar_width/2 - 0.1
        
        # Draw double arrow showing the difference
        if robot_height > console_height:
            # Arrow pointing up (robot higher)
            plt.annotate('', xy=(arrow_x, robot_height), xytext=(arrow_x, console_height),
                        arrowprops=dict(arrowstyle='<->', color='black', lw=2))
            # Add text showing the difference
            diff_text = f'+{robot_height - console_height:.0f}'
            plt.text(arrow_x - 0.15, (robot_height + console_height)/2, diff_text, 
                    rotation=90, va='center', ha='right', fontsize=20, weight='bold')
        else:
            # Arrow pointing down (console higher)
            plt.annotate('', xy=(arrow_x, console_height), xytext=(arrow_x, robot_height),
                        arrowprops=dict(arrowstyle='<->', color='black', lw=2))
            # Add text showing the difference
            diff_text = f'+{console_height - robot_height:.0f}'
            plt.text(arrow_x - 0.15, (robot_height + console_height)/2, diff_text, 
                    rotation=90, va='center', ha='right', fontsize=20, weight='bold')
    ...
    plt.axhline(y=np.sum(data_robot[0, :]), color='red', linestyle='--', linewidth=3)
    plt.axhline(y=np.sum(data_console[0, :]), color='orange', linestyle='--', linewidth=3)
    plt.xticks(x, net_conditions, rotation=45, fontsize=26)
    plt.yticks(fontsize=24)
    plt.ylabel('Motion Length (mm)', fontsize=26)
    plt.title('Motion Length Comparison Across Network Conditions', fontsize=18)
    plt.grid(False)
    
    # Set x-axis limits to reduce side margins
    plt.xlim(-0.7, len(net_conditions) - 0.3)

    plt.legend(handles=[mean_console_line, mean_robot_line, robot_type, console_type] + [pair[0] for pair in legend_handles],
    labels=['Console', 'Robot', 'Robot(Left Bar)', 
            'Console(Right Bar)'] + [pair[1] for pair in legend_handles],
    handler_map={tuple: HandlerTuple(ndivide=None)},
    loc='upper left', framealpha=0.9, fontsize=26, ncol=2)   
