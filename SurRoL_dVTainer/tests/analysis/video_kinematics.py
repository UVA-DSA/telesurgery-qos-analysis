...
robot_data = np.loadtxt("tests/dVTrainer/Data/exp_data_3_new/no_fault/freefault1/robot_sim_data_1.csv", delimiter=",", skiprows=1)
completed_data = np.loadtxt("tests/dVTrainer/Data/exp_data_3_new/no_fault/freefault1/console_data_completed_1.csv", delimiter=",", skiprows=1)
command_data   = np.loadtxt("tests/dVTrainer/Data/exp_data_3_new/no_fault/freefault1/robot_command_data_1.csv", delimiter=",", skiprows=1)
...
# Open video
cap = cv2.VideoCapture("tests/dVTrainer/Data/exp_data_3_new/no_fault/freefault1/free1.mp4")
...
while cap.isOpened():
    ...
    mask_cmd = command_data[:, 0] <= timestamp
    grasper_y = left_grasper[mask_cmd]
    ...
    mask_cmd = command_data[:, 0] <= timestamp
    mask_cmd = command_data[:, 0] <= timestamp
    grasper2_t = command_data[mask_cmd, 0] - command_data[0, 0]
    grasper2_line.set_data(grasper2_t, right_grasper[mask_cmd])
    for ax in axs2:
        ax.legend()
