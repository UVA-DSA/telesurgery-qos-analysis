    ...
    def _step_simulation_task(self, task):
        ...
        if self.demo == None:
            ...
            if task.time - self.time > 1 / 240.0:
                ...
                pos = self.env.psm2.pose_rcm2world(self.env.psm2.get_current_position(), 'tuple')[0]
                print(f"psm2 pos: {pos}")
                ...
                # pos, orn = get_link_pose(self.env.obj_ids['rigid'][0], 0)
                # print(f"red peg pos: {pos}")

                # pos, orn = get_link_pose(self.env.obj_ids['rigid'][1], 0)
                # print(f"green peg pos: {pos}")

                # pos, orn = get_link_pose(self.env.obj_ids['rigid'][2], 0)
                # print(f"blue peg pos: {pos}")

                # pos, orn = get_link_pose(self.env.obj_ids['rigid'][3], 0)
                # print(f"magenta peg pos: {pos}")

                # pos, orn = get_link_pose(self.env.obj_ids['rigid'][4], 0)
                # print(f"yellow peg pos: {pos}")

                # pos, orn = get_link_pose(self.env.obj_ids['rigid'][5], 0)
                # print(f"cyan peg pos: {pos}")
